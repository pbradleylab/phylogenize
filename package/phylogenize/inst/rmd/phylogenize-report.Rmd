...
if (!exists("opts")) {
    if ("options" %in% names(core)) {
        ...
        opts <- phylogenize:::pz.resolve.options()
    }
}

pz.warning(paste(names(core)), .opts=opts)

cache.d <- file.path(opts('out_dir'), 'cache')
dir.create(opts('out_dir'), recursive=TRUE, showWarnings=FALSE)
dir.create(cache.d, recursive=TRUE, showWarnings=FALSE)
knitr::opts_chunk$set(cache.path=cache.d)

if (opts('use_rmd_params')) {
    opts <- do.call(
        phylogenize:::pz.resolve.options,
        c(list(.opts=opts), params)
    )
}

if (is.null(opts('relative_out_dir'))) {
  opts <- phylogenize:::pz.resolve.options(
      .opts=opts,
      relative_out_dir=opts('out_dir')
  )
}
...
```{r show_options, results='asis', echo=FALSE}

o <- opts()
for (i in 1:length(o)) {
	if ((is.null(o[i])) || (o[i] == "")) { o[i] <- "*(empty)*" }
	cat(paste0(" - ", names(o)[i], " = ", o[i]), "\n")
}
...
Below, trees for all taxa with at least `r opts('treemin')`
representatives in the input data are presented. Trees are colored according to
the phenotype chosen (here, `r opts('which_phenotype')` in the environment
 `r opts('which_envir') `).
...
tree_lengths <- vapply(core[["list_pheno"]][["pz.db"]][["trees"]], \(.) length(.[["tip.label"]]), 1L)
if (max(tree_lengths) <= 1) stop("No tree had more than 1 tip")
core[["list_pheno"]][["pz.db"]][["trees"]] <- core[["list_pheno"]][["pz.db"]][["trees"]][tree_lengths > 1]
core[["list_pheno"]][["pz.db"]][["gene.presence"]] <- core[["list_pheno"]][["pz.db"]][["gene.presence"]][names(which(tree_lengths > 1))]
core[["list_pheno"]][["pz.db"]][["species"]] <- core[["list_pheno"]][["pz.db"]][["species"]][names(which(tree_lengths > 1))]
core[["list_pheno"]][["pz.db"]][["ntaxa"]] <- length(names(which(tree_lengths > 1)))
...
pheno.distros <- plot.pheno.distributions(phenotype, pz.db, .opts=opts)
pheno.scale <- get.pheno.plotting.scales(phenotype, pz.db$trees, phenoP, .opts=opts)
plotted.pheno.trees <- plot.phenotype.trees(phenotype, pz.db$trees, pz.db$taxonomy, pheno.scale, .opts=opts)
...
plots <- lapply(seq_along(pheno.distros), function(nm) {
    knitr::knit_expand(text = template, nm = nm)
})

write.table(file=file.path(opts('out_dir'), "phenotype.tab"),
            phenotype,
            sep = '\t')
} else {

pz.message("Skipping phenotypes, as not found (possibly because POMS was used)", .opts=opts)

# Create dummy phenotypes for significant gene plot 
all_species <- Reduce(union, purrr::map(pz.db$trees, \(.) .$tip.label))
...
subtrees <- lapply(pz.db$trees, function(tr) {
  phylogenize::keep.tips(tr, intersect(mapped.observed, pheno.labels))
})
...
subplots <- list()
if (!opts('skip_graphs')) {
       plotted.pheno.subtrees <- plot.phenotype.trees(phenotype, subtrees, pz.db$taxonomy, pheno.scale, .opts=opts)
...
if (is.null(signif_genes)) {
  pz.message("Skipping association summary because no association results were found", .opts=opts)
} else {
  pos.sig.count <- purrr::map_dbl(signif_genes$pos.sig, length)

  if ("neg.sig" %in% names(signif_genes)) {
    neg.sig.count <- purrr::map_dbl(signif_genes$neg.sig, length)
    sig.count <- dplyr::full_join(
      tibble::enframe(pos.sig.count, name = "Taxon", value = "Pos. Sig. Genes"),
      tibble::enframe(neg.sig.count, name = "Taxon", value = "Neg. Sig. Genes"),
      by = "Taxon"
    )
  } else {
    sig.count <- tibble::enframe(pos.sig.count, name = "Taxon", value = "Pos. Sig. Genes")
  }

  phylogenize::getClinDT(sig.count)

  write.csv(
    file = file.path(opts('out_dir'), "pos-sig-thresholded.csv"),
    signif_genes[["pos.sig.descs"]]
  )
  if ("neg.sig" %in% names(signif_genes)) {
    write.csv(
      file = file.path(opts('out_dir'), "neg-sig-thresholded.csv"),
      signif_genes[["neg.sig.descs"]]
    )
  }
  write.csv(
    file = file.path(opts('out_dir'), "all-results.csv"),
...
have particularly extreme phenotypes. Positively-associated genes are plotted
first, then negatively-associated genes.

```{r gene_assoc_heat_maps, warning = FALSE, message = FALSE, echo=FALSE, results="hide", dev = "png", dpi = 300, cache=FALSE}
if (is.null(signif_genes)) {
  pz.message("Skipping gene association heat maps because no association results were found", .opts=opts)
} else {
  for (which_sign in c(1, -1)) {
    if (which_sign==1) {
      if ("phy.with.pos.sigs" %in% signif_genes) {
        which_phy <- signif_genes[["phy.with.pos.sigs"]]
        sig_thresh <- signif_genes[["pos.sig.thresh"]]
      } else {
        which_phy <- signif_genes[["phy.with.sigs"]]
        sig_thresh <- signif_genes[["pos.sig.thresh"]]
      }
    } else {
      if ("phy.with.neg.sigs" %in% signif_genes) {
        which_phy <- signif_genes[["phy.with.neg.sigs"]]
        sig_thresh <- signif_genes[["neg.sig.thresh"]]
      } else {
        break
      }
    }
    valid_keys <- intersect(which_phy, names(plotted.pheno.trees))
    valid_keys <- intersect(valid_keys, names(pz.db$gene.presence))
    valid_keys <- intersect(valid_keys, names(pz.db$trees))

    cluster.plots <- mapply(phylogenize::single.cluster.plot,
                            pz.db$gene.presence[valid_keys],
                            sig_thresh[valid_keys],
                            pz.db$trees[valid_keys],
                            plotted.pheno.trees[valid_keys],
                            valid_keys,
                            MoreArgs=list(.opts=opts),
                            SIMPLIFY = FALSE)
    purrr::walk(cluster.plots, print)
  }
}
...
if (is.null(opts('relative_out_dir'))) {
  r.outdir <- opts('out_dir')
} else {
  r.outdir <- opts('relative_out_dir')
}
for (f in c("phenotype.tab",
            "all-results.csv",
            "pos-sig-thresholded.csv",
            "neg-sig-thresholded.csv",
            "enr-table.csv",
            "enr-overlaps.csv",
  ...
  # got made
  if (file.exists(file.path(opts('out_dir'), f))) {
    cat(paste0(' * [**', f, '**](', fp, ')\n'))
  }
}
```
