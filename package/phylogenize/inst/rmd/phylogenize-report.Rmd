...
### All Taxa
```{r plot_pheno_trees, echo=FALSE, warning = FALSE, results='asis', cache=FALSE}
...
```{r plot_pheno_trees_observed_taxa, echo=FALSE, warning=FALSE, results='asis', warning = FALSE, message = FALSE, cache=FALSE}
...
signif_genes <- get_signif_associated_genes(pz.db, phenotype, do_POMS, p.method)
pos.sig <- data.frame(
  Taxon = names(signif_genes[[4]]),
  `Significant Genes` = sapply(signif_genes[[4]], length),
  row.names = NULL
)
...
valid_keys <- intersect(signif_genes[[6]], names(plotted.pheno.trees))
...
respectively.
```{r enrichment_analysis, warning = FALSE, message = FALSE, echo=FALSE, cache=FALSE, results='axsis'}
do_enrichment <- ("clusterProfiler" %in% rownames(installed.packages()))
if (do_enrichment) {
	table <- clinUtils::getClinDT(get_enrichment_tbls(signif_genes[[2]],
                                    signif_genes[[3]],
                                    pz.db,
                                    signif_genes[[5]],
                                    export=TRUE,
