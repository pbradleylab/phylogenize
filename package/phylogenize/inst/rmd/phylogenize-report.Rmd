...
core[["pz.db"]][["list_pheno"]][["trees"]] <- core[["pz.db"]][["list_pheno"]][["trees"]][tree_lengths > 1]
core[["pz.db"]][["list_pheno"]][["gene.presence"]] <- core[["pz.db"]][["list_pheno"]][["gene.presence"]][names(which(tree_lengths > 1))]
core[["pz.db"]][["list_pheno"]][["species"]] <- core[["pz.db"]][["list_pheno"]][["species"]][names(which(tree_lengths > 1))]
core[["pz.db"]][["list_pheno"]][["ntaxa"]] <- length(names(which(tree_lengths > 1)))
...
pos.sig.count <- purrr::map_dbl(core[["list_signif"]]$pos.sig, length)

if ("neg.sig" %in% names(core[["list_signif"]])) {
  neg.sig.count <- purrr::map_dbl(core[["list_signif"]]$neg.sig, length)
  sig.count <- full_join(
    enframe(pos.sig.count, name = "Taxon", value = "Pos. Sig. Genes"),
...
for (which_sign in c(1, -1)) {
  ...
  cluster.plots <- mapply(single.cluster.plot,
