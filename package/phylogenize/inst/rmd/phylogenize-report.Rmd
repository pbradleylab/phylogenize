...
enrichment.tbl <- multi.enrich(signif,
                               signs,
                               pz.db$g.mappings) %>%
  filter(enr.qval <= 0.25, enr.estimate > 1)
...
check.pryr()
output.enr.table(pretty.enr.tbl)
check.pryr()
...
enr.overlap <- select(enrichment.tbl,
                      ...
                      term,
                      enr.overlap) %>% unnest %>%
               left_join(., pz.db$gene.to.fxn, by=c("enr.overlap"="gene")) %>%
               rename(gene=enr.overlap, description=`function`) %>%
               mutate(effectsize=map2_dbl(phylum, gene, ~results[[.x]][1, .y]))
check.pryr()
...
write.csv(file=file.path(pz.options('out_dir'),
          ...
          arrange(enr.overlap,
                  phylum, cutoff, termset, term, -effectsize))
