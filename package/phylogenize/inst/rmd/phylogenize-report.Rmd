...
check.mem_usage <- function(x) {
	if (pz.options('check_mem_usage')) message(lobstr::cst(obj_size(x)))
}
...
if (pz.options('check_mem_usage')) { library(lobstr) }
if (pz.options('check_mem_usage')) message(lobstr::mem_used() / 1e6)
...
} else if (pz.options("which_phenotype") == "correlation") {
  phenotype <- correl.clr(abd.meta)
  phenoP <- 0
