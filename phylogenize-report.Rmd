...
params:
  ...
  skip_graphs: FALSE
  burst_dir: "/var/www/phylogenize/bin"
---
...
if ((Env == "sample") || (Dset == "sample")) {
  stop("Environment and dataset columns cannot be titled \"sample\"")
}
if (params$input_format == "tabular") {
  abd.mtx <- fastread(file.path(IDir, params$abundance_file), 
    cn = FALSE)
  metadata <- fread(file.path(IDir, params$metadata_file))
  if (!(Env %in% colnames(metadata))) {
    stop(paste("Environment column not found:", Env))
  }
  if (!(Dset %in% colnames(metadata))) {
    stop(paste("Dataset column not found:", Dset))
  }
...
if (length(samples.present) == 0) {
  stop("error: no samples found in both metadata and abundance matrix")
}
...
(This section contains the same trees, but with only the mapped/observed taxa plotted. For the "specificity" phenotype this should be the same.)
...
do.clust.plot <- function(x, y, z, w, nt) {
  ...
  sig.bin <- as.matrix(x[intersect(rownames(x), y), ])
