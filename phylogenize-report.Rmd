...
mapped.observed <- names(which(rowSums(abd.meta$mtx) > 0))
...
plotted.pheno.trees <- plot.phenotype.trees.wrapper(phenotype,
                                                    pz.db,
                                                    pheno.scale)
...
if (!pz.options('skip_graphs')) {
    plot.labeled.phenotype.trees(plotted.pheno.trees, phenotype)
}

write.table(file=file.path(pz.options('output_dir'), "phenotype.tab"),
            phenotype,
            sep = '\t')
...
subtrees <- lapply(pz.db$trees, function(tr) keep.tips(tr, mapped.observed))
plotted.pheno.subtrees <- plot.phenotype.trees(phenotype,
                                               subtrees,
                                               pheno.scale)
if (!pz.options('skip_graphs')) {
    plot.labeled.phenotype.trees(plotted.pheno.subtrees, phenotype)
}
...
pos.sig.descs <- add.sig.descs(pos.sig, pz.db$gene.to.fxn)

pos.sig.thresh <- threshold.pos.sig(pz.db, phy.with.sigs)
pos.sig.thresh.descs <- add.sig.descs(pos.sig.thresh, pz.db$gene.to.fxn)

# recalculate, since some of these may go away
phy.with.sigs <- names(which(sapply(pos.sig.thresh, length) > 0))

write.csv(file = file.path(pz.options('output_dir'), "pos-sig-thresholded.csv"),
          pos.sig.thresh.descs)
rm(pos.sig.thresh.descs)

write.csv(file = file.path(pz.options('output_dir'), "all-results.csv"),
          results.matrix)
rm(results.matrix)
...
enr.table <- generic.make.tables(enrichments,
                                 col.names = c(
                                     ...
                                     "q_value"))
check.pryr()
write.csv(file=file.path(pz.options('output_dir'), "enr-table.csv"), enr.table)
check.pryr()
```
...
enr.overlap.lists <- calc.enr.overlaps(enrichments, results)
check.pryr()
tryCatch({
    enr.overlap <- annotate.nested(enr.overlap.lists,
                                   n.names=c("level",
                                             "phylum",
                                             "process"))
    write.csv(file=file.path(pz.options('output_dir'),
                             "enr-overlaps.csv"),
              enr.overlap)
    write.csv(file=file.path(pz.options('output_dir'),
	...
	pz.message(paste0("Error computing enrichment overlaps: ", e))
})
