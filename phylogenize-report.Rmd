...
all.dsets <- unique(metadata$dataset)
env.number <- sapply(all.envs, function(e) sum(metadata$env == e))
dset.number <- sapply(all.dsets, function(d) sum(metadata$dataset == d))
...
metadata <- metadata[(metadata$env %in% nonsingleton.envs) &
  (metadata$dataset %in% nonsingleton.dsets), ]
abd.mtx <- abd.mtx[, intersect(colnames(abd.mtx), metadata$sample)]




# necessary now that we're using fread
