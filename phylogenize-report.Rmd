...
```{r subtrees, fig.width = 7, fig.height = 20, warning = FALSE, message = FALSE, results="hide", echo=FALSE}
...
for (ps in plotted.pheno.subtrees) {
  rp <- ps$rphy
  tr <- ps$tree
  depth <- max(nodeHeights(rp))
  tIDs <- tax.annot(ps$rphy$tip.label)
  newtr <- (tr %<+% data.frame(
    label = rp$tip.label,
    label2 = tIDs
  )) +
    geom_tiplab(aes(label = label2), size = 0.8) +
    ggplot2::xlim(0, (depth * 1.5))
  print(newtr)
}
...
colnames(enr.table) <- c("Gene_significance", "Subsystem_level",
...
enr.table <- data.frame(enr.table)
